	...
	<body>
		...
		<div class="float-right-profile">
			...
			<?php
				...
				<div class="profile-box-profile-link">
					<a href="Profile.php"><p class="link"><strong>Profile</strong></p></a>
				</div>
				*/
			?>
		</div>
		<div class="float-middle-posts">
			<div class="page-title"><h4>View Post</h4>
			</div>

			<?php
				$query_string = "SELECT * FROM post WHERE post_id = ".$_SESSION['post_id'];
				$home_posts = $mysqli->query($query_string); 
				$post = $home_posts->fetch_assoc();

				echo '<div class="post-container">';
				echo '<div class="post-print">';
				echo '<img class="post-profile-photo" src="stockProfile.jpg">';
				echo '<div class="post-title"><h4>'.$post['username'].' | '.$post['title'].'</h4>
					</div>';
				...
				*/
				...
				$query_string = "SELECT * FROM comment WHERE post_id = ".$_SESSION['post_id'];
				$all_comments = $mysqli->query($query_string);

				while ($comment = $all_comments->fetch_assoc()) {
					echo '<div class="post-container">';
					echo '<div class="post-print">';
					echo '<img class="post-profile-photo" src="stockProfile.jpg">';
					echo '<div class="post-title"><h4>'.$comment['username'].'</h4>';
					echo '</div>';
					echo '<div class="post-body-text"><p>'.$comment['comment_body'].'</p>
					</div>';
					echo '</div>';
					echo '</div>';
				}
				...
				<div class="post-print">
					<img class="post-profile-photo" src="stockProfile.jpg">
					<div class="post-title"><h4>Username</h4>
					</div>
					<div class="post-body-text"><p>Something something dark side</p>
					</div>
				</div>
			...
			<div class="post-container">
				<div class="post-print">
					...
					$comment_link = "WriteComment.php?post_id=".$_SESSION['post_id']."&username=".$_SESSION['username'];
					echo '<a href="'.$comment_link.'"><p class="create-post-font"><strong>+ Write a new comment</strong></p></a></div>';
					echo '</div>';

					/*<div class="create-post">
				<a href="WriteComment.php"><p class="create-post-font"><strong>+ Write a new comment</strong></p></a>
			</div>
			</div>*/
			?>
